  ...
  return (
    <div className="bg-card-dark p-4 rounded-lg shadow-md">
      <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-700">
        ...
        <button
          className="text-gray-400 hover:text-primary lg:hidden"
          onClick={() => setIsMobileNavOpen(!isMobileNavOpen)}
          aria-expanded={isMobileNavOpen}
          aria-controls="feed-nav-content"
          aria-label="Toggle feed navigation"
        >
          <span className="material-icons text-xl">{isMobileNavOpen ? 'unfold_less' : 'unfold_more'}</span>
        </button>
      </div>
      <div id="feed-nav-content" className={`${isMobileNavOpen ? 'block' : 'hidden'} lg:block`}>
